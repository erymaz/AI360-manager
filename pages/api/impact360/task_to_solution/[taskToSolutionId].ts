...
async function handler(
    ...
    const {taskToSolutionId} = req.query;
    ...
    if (req.method === "PUT") {
        try {
            await prisma.task_to_solution.findFirstOrThrow({
                where: {id: String(taskToSolutionId)},
            });
            const data = req.body;
            ...
            const task_to_solution = await prisma.task_to_solution.update({
                where: {id: String(taskToSolutionId)},
                data: req.body,
            });
        ...
        try {
            ...
            const task_to_solution = await prisma.task_to_solution.delete({
                where: {id: String(taskToSolutionId)},
            });
            return res.status(200).json({
                task_to_solution,
                message: "Task to solution deleted successfully",
            });
        } catch (e: any) {
            return res.status(500).json({
                error: e.toString(),
