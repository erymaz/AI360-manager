...
const taskToSolutionReviewSchema = Joi.object({
    task_to_solution_id: Joi.string(),
...
async function handler(
    ...
    const {task_to_solution_review_id} = req.query;
    ...
    if (req.method === "PUT") {
        try {
            await prisma.task_to_solution_reviews.findFirstOrThrow({
                where: {id: String(task_to_solution_review_id)},
            });
            const data = req.body;
            await taskToSolutionReviewSchema.validateAsync(data);
            if (req.body.task_to_solution_id) {
                await prisma.task_to_solution.findFirstOrThrow({
                    where: {id: req.body.task_to_solution_id},
                });
            }
            ...
            const task_to_solution_review =
                await prisma.task_to_solution_reviews.update({
                    where: {id: String(task_to_solution_review_id)},
                    data: req.body,
                });
        ...
        try {
            ...
            const organization = await prisma.task_to_solution_reviews.delete({
                where: {id: String(task_to_solution_review_id)},
            });
            return res.status(200).json({
                organization,
                message: "Task to solution review deleted successfully",
            });
        } catch (e: any) {
            return res.status(500).json({
                error: e.toString(),
