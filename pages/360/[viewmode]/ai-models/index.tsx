import {useEffect, useState} from "react";
import {InferGetServerSidePropsType} from "next";
import {useRouter} from "next/router";
import styled from "@emotion/styled";
import {Paper, TablePagination} from "@mui/material";
import {StyledEngineProvider} from "@mui/material/styles";

import ModelsTable from "@/components/waipify360/models/ModelsTable";
import ModelFilter from "@/components/waipify360/models/ModelFilter";
import CreateNewModel from "@/components/waipify360/models/CreateNewModel";
import Layout360 from "@/components/layouts/Layout360";
import { request } from "@/helpers/request";
import { withRequiredAuthentication } from "@/lib/services/auth";

export const getServerSideProps = withRequiredAuthentication(
    async (ctx, {organisation, organization360, locale, messages}) => {
        return {
            props: {
                companyName: organisation.companyName,
                organization360,
                locale,
                messages
            },
        };
    }
);

const Wap360AdminTitle = styled.h3`
    font-size: 18px;
    font-weight: 600;
    line-height: 24px;
    color: #1a262d;
`;

const Wap360AdminSearch = styled.div`
    padding: 10px 0px;

    & input {
        border-radius: 8px;
        border: 1px solid #d7dbe0;
        background: #fff;
        padding: 8px 13px;
        padding-left: 32px;
        color: #000;
        font-size: 16px;
        font-style: normal;
        font-weight: 400;
        line-height: 24px;
        width: 100%;
        max-width: 762px;
        background-image: url(data:image/svg+xml;base64,);
        background-position: left 6px center;
        background-repeat: no-repeat;

        ::placeholder {
            color: #9ca5af;
        }

        :focus {
            outline: none;
        }
    }

    .wap-drap-add {
        display: none;
    }

    & button {
        border-radius: 8px;
        background: #1f75ff;
        padding: 10px 11px 10px 13px;
        color: #fff;
        font-size: 14px;
        font-style: normal;
        font-weight: 600;
        line-height: 20px;
        border: none;
        cursor: pointer;
        margin-left: 26px;
        position: relative;
        overflow: hidden;
        @media (max-width: 1114px) {
            margin-left: 10px;
        }
        @media (max-width: 1098px) {
            margin-left: 0;
            margin-top: 10px;
            min-width: 216px;
        }

        & span {
            width: 51px;
            height: 40px;
            background-image: url("data:image/svg+xml;base64,");
            display: inline-block;
            background-repeat: no-repeat;
            background-position: center;
            margin-left: 15px;
            background-color: #1e428a;
            position: absolute;
            top: 0;
            right: 0;
        }
    }
`;

const WapTopOrder = styled.div`
    display: flex;
    align-items: center;
    gap: 10px;
`;

const Wap360adminTable = styled(Paper)`
    margin-top: 20px;
`;

const AiModels = ({
  companyName,
  organization360,
}: InferGetServerSidePropsType<typeof getServerSideProps>) => {
  const router = useRouter();
  const [models, setModels] = useState<any>(false);
  const [modelModal, setModelModal] = useState(false);
  const [selectedModelId, setSelectedModelId] = useState<String | null>(
    null
  );
  const [searchTimeOut, setSearchTimeOut] = useState<any>("");
  const [search, setSearch] = useState("");
  const [pageData, setPageData] = useState({
    page: 0,
    perpage: 25,
  });

    const [filterData, setFilterData] = useState<any>({
        provider_ids: [],
        model_ids: [],
        type_ids: [],
        avg_sft: [null, null],
        avg_raiscore: [null, null],
        search_val: "",
    });

    useEffect(() => {
        loadModels(organization360);
    }, [pageData]);

    const handlePageDataChange = (value?: any, type?: any) => {
        const details: any = {
            [type]: value,
        };
        if (type === "perpage") {
            details["page"] = 0;
        }

        setPageData({
            ...pageData,
            ...details,
        });
    };

    const loadModels = async (org: any) => {
        const param: any = {};
        for (const filfield in filterData) {
            if (filfield != "avg_sft" && filfield != "avg_raiscore") {
                if (filterData[filfield]) {
                    param[filfield] = filterData[filfield];
                }
            } else {
                if (filterData[filfield][0] || filterData[filfield][1]) {
                    param[filfield] = filterData[filfield];
                }
            }
        }

        let url = `/api/impact360/ivmmodel?organization_creator_id=${org?.creator_id}`;

        if (organization360.is_superadmin) {
            url += "&view=admin";
        } else if (organization360.is_supervisor) {
            url += "&view=supervisor";
        }

        if (pageData.perpage) {
            url = `${url}&page=${pageData.page}&limit=${pageData.perpage}`;
        }

        return request<{ cases?: any }>(url, {
            method: "POST",
            body: param,
        }).then((res: any) => {
            if (res) {
                setModels(res);
            }
        });
    };

    const handleModelModal = (isOpen: boolean) => {
        setSelectedModelId(null);
        setModelModal(isOpen);
    }

    const handleEditModel = (id: string) => {
        setSelectedModelId(id);
        setModelModal(true);
    };

    const handleSearch = (event: React.ChangeEvent<HTMLInputElement>) => {
        const value = event && event.target.value;
        setSearch(value);
        handleFilterDataChange(value, "search_val");
    };

    const handleFilterDataChange = (value: any, type: any) => {
        if (searchTimeOut) {
            clearInterval(searchTimeOut);
        }
        const mytimeOut = setTimeout(() => {
            setPageData({
                ...pageData,
                page: 0,
            });

            if (type === "avg_sft_min") {
                type = "avg_sft";
                value = [value, filterData.avg_sft[1]];
            } else if (type === "avg_sft_max") {
                type = "avg_sft";
                value = [filterData.avg_sft[0], value];
            } else if (type === "avg_raiscore_min") {
                type = "avg_raiscore";
                value = [value, filterData.avg_raiscore[1]];
            } else if (type === "avg_raiscore_max") {
                type = "avg_raiscore";
                value = [filterData.avg_raiscore[0], value];
            }

            setFilterData({
                ...filterData,
                [type]: value,
            });
        }, 500);
        setSearchTimeOut(mytimeOut);
    };

  return (
    <>
      <div className="p-6">
        <WapTopOrder>
          <Wap360AdminTitle>Models</Wap360AdminTitle>
          <Wap360AdminSearch>
            <button onClick={() => handleModelModal(true)}>
              Add New Model +
            </button>
          </Wap360AdminSearch>
        </WapTopOrder>
        <Wap360AdminSearch>
          <input
            type="text"
            placeholder="Search by model name or keyword"
            value={search}
            onChange={handleSearch}
          />
        </Wap360AdminSearch>

        <ModelFilter
          organization360={organization360}
          handleFilterDataChange={handleFilterDataChange}
        />

        <StyledEngineProvider injectFirst>
          <Wap360adminTable sx={{ width: "100%", overflow: "hidden" }}>
            <ModelsTable
              model={models}
              handleEditModel={handleEditModel}
            />

            <TablePagination
              rowsPerPageOptions={[10, 25, 50]}
              component="div"
              count={models?.count || 0}
              rowsPerPage={pageData.perpage}
              page={pageData.page}
              onPageChange={(event: any, value: any) =>
                handlePageDataChange(value, "page")
              }
              onRowsPerPageChange={(event) =>
                handlePageDataChange(event.target.value, "perpage")
              }
            />
          </Wap360adminTable>
        </StyledEngineProvider>
      </div>
      <CreateNewModel
        open={modelModal}
        impOrganization={organization360}
        setIsOpen={handleModelModal}
        modelId={selectedModelId}
        isEdit={false}
      />
    </>
  );
}

AiModels.getLayout = function getLayout(page: React.ReactElement) {
  return (
    <Layout360>
      {page}
    </Layout360>
  );
}

export default AiModels;
