"use client";

import { useEffect, useState } from "react";
import { InferGetServerSidePropsType } from "next";
import { useRouter } from "next/router";
import styled from "@emotion/styled";
import { Paper, TablePagination } from "@mui/material";
import { StyledEngineProvider } from "@mui/material/styles";

import InventoryMasterTable from "@/components/waipify360/solutions/InventoryMasterTable";
import UseCaseFilter from "@/components/waipify360/solutions/ProviderFilter";
import CreateNewProvider from "@/components/waipify360/solutions/ProviderModel";
import { request } from "@/helpers/request";
import { withRequiredAuthentication } from "@/lib/services/auth";
import Layout360 from "@/components/layouts/Layout360";

export const getServerSideProps = withRequiredAuthentication(
    async (ctx, {organisation, organization360, locale, messages}) => {
        return {
            props: {
                companyName: organisation.companyName,
                organization360,
                locale,
                messages
            },
        };
    }
);

const Wap360AdminTitle = styled.h3`
    font-size: 18px;
    font-weight: 600;
    line-height: 24px;
    color: #1a262d;
`;

const Wap360AdminSearch = styled.div`
    padding: 10px 0px;

    & input {
        border-radius: 8px;
        border: 1px solid #d7dbe0;
        background: #fff;
        padding: 8px 13px;
        padding-left: 32px;
        color: #000;
        font-size: 16px;
        font-style: normal;
        font-weight: 400;
        line-height: 24px;
        width: 100%;
        max-width: 762px;
        background-image: url(data:image/svg+xml;base64,);
        background-position: left 6px center;
        background-repeat: no-repeat;

        ::placeholder {
            color: #9ca5af;
        }

        :focus {
            outline: none;
        }
    }

    .wap-drap-add {
        display: none;
    }

    & .active-drop {
        background-color: #fcfdff;
        border-radius: 6px;
        position: absolute;
        left: 900px;
        padding: 20px 11px;
        min-width: 206px;
        list-style: none;
        margin-top: 5px;
        margin-bottom: 0;
        display: block;
        z-index: 99;
        @media (max-width: 1098px) {
            left: 110px;
        }

        & li {
            margin-bottom: 16px;
            font-size: 14px;
            font-weight: 500;
            line-height: 16px;
            color: #1d5bd8;
            text-decoration: underline;
            cursor: pointer;

            :last-child {
                margin-bottom: 0px;
            }
        }
    }

    & button {
        border-radius: 8px;
        background: #1f75ff;
        padding: 10px 11px 10px 13px;
        color: #fff;
        font-size: 14px;
        font-style: normal;
        font-weight: 600;
        line-height: 20px;
        border: none;
        cursor: pointer;
        margin-left: 26px;
        position: relative;
        overflow: hidden;
        @media (max-width: 1114px) {
            margin-left: 10px;
        }
        @media (max-width: 1098px) {
            margin-left: 0;
            margin-top: 10px;
            min-width: 216px;
        }

        & span {
            width: 51px;
            height: 40px;
            background-image: url("data:image/svg+xml;base64,");
            display: inline-block;
            background-repeat: no-repeat;
            background-position: center;
            margin-left: 15px;
            background-color: #1e428a;
            position: absolute;
            top: 0;
            right: 0;
        }
    }
`;

const WapTopOrder = styled.div`
    display: flex;
    align-items: center;
    gap: 10px;
`;

const Wap360adminTable = styled(Paper)`
    margin-top: 20px;
`;

const AiSolutions = ({
  companyName,
  organization360,
}: InferGetServerSidePropsType<typeof getServerSideProps>) => {
  const router = useRouter();
  const [isActive, setIsActive] = useState(false);
  const [providers, setProviders] = useState<any>(false);
  const [total, setTotal] = useState(0);
  const [providerModel, setProviderModel] = useState(false);
  const [selectedProviderId, setSelectedProviderId] = useState<String | null>(
    null
  );
  const [searchTimeOut, setSearchTimeOut] = useState<any>("");
  const [search, setSearch] = useState("");
  const [pageData, setPageData] = useState({
    page: 0,
    perpage: 25,
  });

    const [filterData, setFilterData] = useState<any>({
        provider_ids: [],
        solution_ids: [],
        type_ids: [],
        avg_sft: [null, null],
        avg_raiscore: [null, null],
        search_val: "",
        createdby_ids: [],
    });

    useEffect(() => {
        loadProviders(organization360);
    }, [pageData]);

    const handleClick = (event: React.MouseEvent<HTMLElement>) => {
        event.stopPropagation();
        // setIsActive((current) => !current);
    };

    const handlePageDataChange = (value?: any, type?: any) => {
        const details: any = {
            [type]: value,
        };
        if (type === "perpage") {
            details["page"] = 0;
        }

        setPageData({
            ...pageData,
            ...details,
        });
    };

    function handleProviderModel(value: boolean, type?: boolean) {
        if (type) {
            loadProviders(organization360);
        }
        setProviderModel(value);
        setSelectedProviderId(null);
    }

    const loadProviders = async (org: any) => {
        const param: any = {};
        for (const filfield in filterData) {
            if (filfield != "avg_sft" && filfield != "avg_raiscore") {
                if (filterData[filfield]) {
                    param[filfield] = filterData[filfield];
                }
            } else {
                if (filterData[filfield][0] || filterData[filfield][1]) {
                    param[filfield] = filterData[filfield];
                }
            }
        }

        let url = `/api/impact360/ivmprovider?organization_creator_id=${org?.creator_id}`;

        if (organization360.is_supervisor) {
            url += "&view=supervisor";
        } else {
            url += "&view=admin";
        }

        if (pageData.perpage) {
            url = `${url}&page=${pageData.page}&limit=${pageData.perpage}`;
        }

        return request<{
            providers?: any,
            count: number,
        }>(url, {
            method: "POST",
            body: param,
        }).then((res: any) => {
            if (res) {
                setProviders(res.providers);
                setTotal(res.count || 0);
            }
        });
    };

    const handleEditProvider = (id: string) => {
        setProviderModel(true);
        setSelectedProviderId(id);
    };

    const handleSearch = (event: React.ChangeEvent<HTMLInputElement>) => {
        const value = event && event.target.value;
        setSearch(value);
        handleFilterDataChange(value, "search_val");
    };

    const handleFilterDataChange = (value: any, type: any) => {
        if (searchTimeOut) {
            clearInterval(searchTimeOut);
        }
        const mytimeOut = setTimeout(() => {
            setPageData({
                ...pageData,
                page: 0,
            });

            if (type === "avg_sft_min") {
                type = "avg_sft";
                value = [value, filterData.avg_sft[1]];
            } else if (type === "avg_sft_max") {
                type = "avg_sft";
                value = [filterData.avg_sft[0], value];
            } else if (type === "avg_raiscore_min") {
                type = "avg_raiscore";
                value = [value, filterData.avg_raiscore[1]];
            } else if (type === "avg_raiscore_max") {
                type = "avg_raiscore";
                value = [filterData.avg_raiscore[0], value];
            }

            setFilterData({
                ...filterData,
                [type]: value,
            });
        }, 500);
        setSearchTimeOut(mytimeOut);
    };

  return (
    <>
      <div className="p-6">
        <WapTopOrder>
          <Wap360AdminTitle>Providers</Wap360AdminTitle>
          <Wap360AdminSearch>
            <button onClick={() => handleProviderModel(true)}>
              Add New Provider +
            </button>
            <ul className={isActive ? "active-drop" : "wap-drap-add"}>
              {/* <li> Add new subcase</li>
            <li> Add new task </li> */}
            </ul>
          </Wap360AdminSearch>
        </WapTopOrder>
        <Wap360AdminSearch>
          <input
            type="text"
            placeholder="Search by provider or solution name or keyword"
            value={search}
            onChange={handleSearch}
          />
        </Wap360AdminSearch>

        <UseCaseFilter
          organization360={organization360}
          handleFilterDataChange={handleFilterDataChange}
        />

        <StyledEngineProvider injectFirst>
          <Wap360adminTable sx={{ width: "100%", overflow: "hidden" }}>
            <InventoryMasterTable
              providers={providers}
              organization={organization360}
              handleEditProvider={handleEditProvider}
            />

            <TablePagination
              rowsPerPageOptions={[10, 25, 50]}
              component="div"
              count={total || 0}
              rowsPerPage={pageData.perpage}
              page={pageData.page}
              onPageChange={(event: any, value: any) =>
                handlePageDataChange(value, "page")
              }
              onRowsPerPageChange={(event) =>
                handlePageDataChange(event.target.value, "perpage")
              }
            />
          </Wap360adminTable>
        </StyledEngineProvider>
      </div>
      <CreateNewProvider
        open={providerModel}
        impOrganization={organization360}
        handleCaseModel={handleProviderModel}
        provider_id={selectedProviderId}
      />
    </>
  );
}

AiSolutions.getLayout = function getLayout(page: React.ReactElement) {
  return (
    <Layout360>
      {page}
    </Layout360>
  );
}

export default AiSolutions;
